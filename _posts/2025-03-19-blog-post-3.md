...
{}            &   { = \log{ \int q(c,t,m|x,s)  p(x|c,t,s) dcdtdm}  - \log{ \int q(c,t,m|x,s)  \frac{ q(m|t) \ q(c,t|x,s) }{ p(c,t|s) } dcdtdm}  +  \log{ \int  \  q(m|t) \ q(t|x,s)  dt }   } \\
{}            &   { = \log{ \int q(c,t,m|x,s)  p(x|c,t,s) dcdtdm} - \log{ \int q(c,t|x,s)  \frac{q(c,t|x,s) }{ p(c,t|s)} dcdt} - \log{ \int q(m|t) \ q(m,t|x,s)  dtdm} +    \log{ \int \ q(m|t) \ q(t|x,s)  dt} } \\
{}            &   { = \log{ \int q(c,t,m|x,s)  p(x|c,t,s) dcdtdm} - \log{ \int q(c,t|x,s)  \frac{ q(c,t|x,s) }{ p(c,t|s) } dcdt} - \log{ \int  q(t|x,s)   dt} +    \log{ \int  \ q(m|t) \ q(t|x,s) dt } } \\
{}             &   { \geq  \int q(c,t,m|x,s)  \log{p(x|c,t,s)} dcdtdm -  \int q(c,t|x,s) \log{ \frac{  q(c,t|x,s) }{ p(c,t|s) } } dcdt +  \int  q(t|x,s) \log{ q(m|t)} dt} & { \textbf{ELBO} }\\
...
<p>\begin{array}{ll}
 { {D}_{KL}(q(t,c|x,s)||p(t,c)) } &{  = \left\{ 
                                              {\begin{array}{l} 
                                                {  {D}_{KL}(q(c|x,s)||p(c)) +  {D}_{KL}(q(t|x,s)||p(t))   \, \ if\ cell\ n\ is\ target\ KO\ cell } \\
                                                {  {D}_{KL}(q(c|x,s)||p(c)) +  \textbf{W}^{2}(q(t|x,s), \delta (t=0) ) \, \ if\ cell\ n\ is\ non-targeting\ cell } \\
                                                \end{array}</p>
                                              }
                                        \right.
                            } \\
  {   \textbf{W}^{2}(q(t|x,s), \delta (t=0) )   } & {  =  \sum{ \sigma_{t}(x,s)^{2} } + \sum{ \mu_{t}(x,s)^{2} }  } \\
\end{array}</p>
