...
class PyroVAE( scvi.module.base.PyroBaseModuleClass):
    ...
    def model(self, data_background, data_target, l_mu, l_sig):
        ...
        with pyro.plate('cellXgene', x.shape[0]), poutine.scale( scale=1/(x.shape[0] *self.n_genes) ):
...
写本模型，涉及到了部分scVI和pyro框架，学习了一部分基础。scVI, pytorch lightning, pyro等框架在单细胞领域是非常重要的工具，接下来可以深入学习一下相关的知识，并且整理分享。
