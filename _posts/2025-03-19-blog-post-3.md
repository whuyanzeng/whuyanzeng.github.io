...
{}            &   { = \mathbb{E}_{q(c,t,m|x,s)} \log{ p(x|c,t,s)} - \int q(c,t,m|x,s) \log \frac{ q(m|x,s,t) \ q(c|x,s) \ q(t|x,s) }{ p(c|s) \ p(t|s) } dcdtdm } \\
{}           &   { = \mathbb{E}_{q(c,t,m|x,s)} \log{ p(x|c,t,s) } - {D}_{KL}(q(c|x,s)||p(c|s)) - {D}_{KL}(q(t|x,s)||p(t|s))  -  \int q(t,m|x,s) \log{ q(m|x,s,t) }  dtdm   } \\
              \\
