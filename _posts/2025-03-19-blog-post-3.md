...
<p>\begin{array}{ll}
{ \log{ p(x|s)} + \log{q(m|x,s)} }   &  { = \log{\int p(x,c,t|s)dcdt}  + \log{\int q(m|t) \ q(t|x,s)dt} } \\
{}             &   { = \log{ \int q(c,t,m|x,s)  \frac{p(x|c,t,s) \ p(c,t|s)   }{q(c,t,m|x,s) } dcdtdm } + \log{ \int    q(m|t) q(t|x,s) dt }  } \\
{}            &   { = \log{ \int q(c,t,m|x,s)  p(x|c,t,s) dcdtdm  - \log{ \int q(c,t,m|x,s)  \frac{ q(m|t) \ q(c|x,s) \ q(t|x,s) }{ p(c|s) \ p(t|s) } dcdtdm}  +  \log{ \int    q(m|t) q(t|x,s) dt }   } \\
{}            &   { = \log{ \int q(c,t,m|x,s)  p(x|c,t,s) dcdtdm - \log{ \int q(c,t|x,s)  \frac{ q(c|x,s) \ q(t|x,s) }{ p(c|s) \ p(t|s) } dcdt} - \log{ \int q(m|t) \ q(t|x,s) \  q(m|t)  dtdm} +    \log{ \int  q(m|t) q(t|x,s) dt} } \\
{}            &   { = \log{ \int q(c,t,m|x,s)  p(x|c,t,s) dcdtdm - \log{ \int q(c,t|x,s)  \frac{ q(c|x,s) \ q(t|x,s) }{ p(c|s) \ p(t|s) } dcdt} - \log{ \int  q(t|x,s)   dt} +    \log{ \int   q(m|t) q(t|x,s) dt } } \\
