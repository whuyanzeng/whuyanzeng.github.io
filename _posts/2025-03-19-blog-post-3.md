...
------
<p>\begin{array}{ll}
{ p(x, c, t|s) } & { <p>\begin{array}{l}
                        { = p(x|c, t, s)p(c,t|s) } \\
                        { = p(x|c, t, s)p(c)p(t) } \\
                        \end{array}</p>
                      } \\
{ q(c,t,m|x,s) }   & {  <p>\begin{array}{l}
                        { = q(c,t|x,s) q(m|s,t) } \\
                        { = q(t|x,s) q(c|x,s) q(m|s,t) } \\
                        \end{array}</p>
                    } \\
{ log p(x|s) }   & {  <p>\begin{array}{l}
                    { = log \int p(x,c,t|s)dcdt } } \\
                    ...
                    { = \mathbb{E}_{q(c,t,m|x,s) log p(x|c,t,s)} - \int q(c,t,m|x,s) log \frac{q(t|x,s) q(c|x,s) q(m|s,t)}{ p(c,t|s)} dcdtdm } \\
