...
\\[ {\ell _n} & {\sim {\mathrm{log}}\,{\mathrm{normal}} ( {\ell _\mu ,\ell _\sigma ^2})} \\] 
\\[ {\rho _n} & { = f_w( {z_n,s_n})} \\ {w_{ng}} & {\sim {\mathrm{Gamma}}( {\rho _n^g,\theta } )} \\]
\\[ {y_{ng}} & {\sim {\mathrm{Poisson}}( {\ell _nw_{ng}} )} \\]
\\[ {h_{ng}} & {\sim {\mathrm{Bernoulli}}( {f_h^g\left( {z_n,s_n} \right)} )} \\]
\\[ {x_{ng}} & { = \{  {y_{ng}\,\text{if}\,h_{ng} = 0} \\ {0\,\text{otherwise}}\,}  \\]
...
```
![Umap of target latent variable](https://oldcountlost.github.io/images/contrastiveVI_pyro.target.png)
...
```
![Umap of target latent variable](https://oldcountlost.github.io/images/contrastiveVI_pyro.control.png)
