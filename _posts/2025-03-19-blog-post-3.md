...
<p>\begin{array}{ll}
{ \log{ p_{\theta}(x|s)} + \log{q_{\phi}(m|x,s)} }   &  { = \log{\int p_{\theta}(x,c,t|s)dcdt}  + \log{\int q_{\phi}(m,t|x,s) dt} } \\
{}             &   { = \log{ \int q_{\phi}(c,t|x,s)  \frac{p_{\theta}(x|c,t,s) \ p_{\theta}(c,t|s) }{q_{\phi}(c,t|x,s)} dcdt } + \log{ \int   \ q_{\phi}(m|t) \ q_{\phi}(t|x,s) dt }  } \\
  {}             &   { = \log{ \int q_{\phi}(c,t|x,s)  \frac{p_{\theta}(x|c,t,s) \ p_{\theta}(c,t|s) }{ q_{\phi}(c,t|x,s) } dcdt } + \log{ \int   \ q_{\phi}(m|t) \ q_{\phi}(t|x,s) dt }  } \\
  {}             &   { \geq   \underbrace{  \int q_{\phi}(c,t|x,s)  \log{ \frac{ p_{\theta}(x|c,t,s) \ p_{\theta}(c,t|s)}{  q_{\phi}(c,t|x,s) }  } dcdt +  \int  q_{\phi}(t|x,s) \log{ q_{\phi}(m|t)} dt  }_\textbf{ ELBO (根据Jensen 不等式) }  } \\
{}            &   { = \int q_{\phi}(c,t|x,s) \log{  p_{\theta}(x|c,t,s) dcdt}  +  \int  q_{\phi}(c,t|x,s)  \log{ \frac{ p_{\theta}(c,t|s) }{ q_{\phi}(c,t|x,s) } dcdt}  +  \int  q_{\phi}(t|x,s) \log{  q_{\phi}(m|t)   dt }   } \\
  {}            &   { = \mathbb{E}_{q_{\phi}(c,t|x,s)} \log{ p_{\theta}(x|c,t,s)} - \int q_{\phi}(c,t|x,s) \log{ \frac{  q_{\phi}(c,t|x,s) }{ p_{\theta}(c,t|s) } } dcdt   +  \mathbb{E}_{q_{\phi}(t|x,s)} \log{ q_{\phi}(m|t) }  } \\
 {}            &   { = \underbrace{\mathbb{E}_{q_{\phi}(c,t|x,s)} \log{ p_{\theta}(x|c,t,s)} }_{ {- Loss_{reconst}}  } + \underbrace{\left( { - {D}_{KL}(q_{\phi}(c,t|x,s)||p_{\theta}(c,t)) } \right)}_{ - Loss_{KLD} }  + \underbrace{ \mathbb{E}_{q_{\phi}(t|x,s)} \log{ q_{\phi}(m|t) }   }_{- Loss_{Classify} }  } \\
              \\
\end{array}</p>
