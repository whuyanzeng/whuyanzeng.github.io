...
对于实验设计矩阵\\(D_{np}\\), \\[D_{np} = 1,\  if\ the \ \textit{n}-th \ cell \ is \ in \ perturbation \ group \  \textit{p}\\]
\\[ zt_n \sim {\mathrm{Normal}}( {0,I} )  \\]
\\[ zt_n = 0, \ if \ \sum_{p}D_{np}=0 \\]
...
\\[ Loss_{reconst} =  \sum_{n}\sum_{g}(x_{ng}- \hat{x}_{ng})^2\\]
\\[ Loss_{Classify} = CE \( D,\hat{D} \) \\]
