...
{}             &   { \geq  \int q(c,t,m|x,s)  \log{p(x|c,t,s)} dcdtdm -  \int q(c,t|x,s) \log{ \frac{ q(c|x,s) \ q(t|x,s) }{ p(c|s) \ p(t|s) }} dcdt +  \int  q(t|x,s) \log{ q(m|t)} dt} & { (ELBO) }\\
{}            &   { = \mathbb{E}_{q(c,t,m|x,s)} \log{ p(x|c,t,s)} - \int q(c,t|x,s) \log{ \frac{  q(c|x,s) \ q(t|x,s) }{ p(c|s) \ p(t|s) } dcdt} +  \mathbb{E}_{q(t|x,s)} \log{ q(m|t) } } \\
{}           &   { = \mathbb{E}_{q(c,t,m|x,s)} \log{ p(x|c,t,s) } - {D}_{KL}(q(c|x,s)||p(c|s)) - {D}_{KL}(q(t|x,s)||p(t|s))  +   \mathbb{E}_{q(t|x,s)} \log{ q(m|t) }  } \\
{}           &   { = \mathbb{E}_{q(c,t,m|x,s)} \log{ p(x|c,t,s) } - {D}_{KL}(q(c|x,s)||p(c)) - {D}_{KL}(q(t|x,s)||p(t))  +  \mathbb{E}_{q(t|x,s)} \log{ q(m|t) } } \\
              \\
