...
<p>\begin{array}{ll}
{ \log{ {p}_{\theta}(x|s)} + \log{ {q}_{\phi}(m|x,s)} }   &  { = \log{\int {p}_{\theta}(x,c,t|s)dcdt}  + \log{\int {q}_{\phi}(m,t|x,s) dt} } \\
{}             &   { = \log{ \int {q}_{\phi}(c,t|x,s)  \frac{ {p}_{\theta}(x|c,t,s) \ {p}_{\theta}(c,t|s) }{{q}_{\phi}(c,t|x,s)} dcdt } + \log{ \int   \ {q}_{\phi}(m|t) \ {q}_{\phi}(t|x,s) dt }  } \\
  {}             &   { = \log{ \int {q}_{\phi}(c,t|x,s)  \frac{{p}_{\theta}(x|c,t,s) \ {p}_{\theta}(c,t|s) }{ {q}_{\phi}(c,t|x,s) } dcdt } + \log{ \int   \ {q}_{\phi}(m|t) \ {q}_{\phi}(t|x,s) dt }  } \\
  {}             &   { \geq   \underbrace{  \int {q}_{\phi}(c,t|x,s)  \log{ \frac{ {p}_{\theta}(x|c,t,s) \ {p}_{\theta}(c,t|s)}{  {q}_{\phi}(c,t|x,s) }  } dcdt +  \int  {q}_{\phi}(t|x,s) \log{ {q}_{\phi}(m|t)} dt  }_\textbf{ ELBO (根据Jensen 不等式) }  } \\
{}            &   { = \int {q}_{\phi}(c,t|x,s) \log{  {p}_{\theta}(x|c,t,s) dcdt}  +  \int  {q}_{\phi}(c,t|x,s)  \log{ \frac{ {p}_{\theta}(c,t|s) }{ {q}_{\phi}(c,t|x,s) } dcdt}  +  \int  {q}_{\phi}(t|x,s) \log{  {q}_{\phi}(m|t)   dt }   } \\
  {}            &   { = \mathbb{E}_{{q}_{\phi}(c,t|x,s)} \log{ {p}_{\theta}(x|c,t,s)} - \int {q}_{\phi}(c,t|x,s) \log{ \frac{  {q}_{\phi}(c,t|x,s) }{ {p}_{\theta}(c,t|s) } } dcdt   +  \mathbb{E}_{{q}_{\phi}(t|x,s)} \log{ {q}_{\phi}(m|t) }  } \\
 {}            &   { = \underbrace{\mathbb{E}_{{q}_{\phi}(c,t|x,s)} \log{ {p}_{\theta}(x|c,t,s)} }_{ {- Loss_{reconst}}  } + \underbrace{\left( { - {D}_{KL}({q}_{\phi}(c,t|x,s)||{p}_{\theta}(c,t)) } \right)}_{ - Loss_{KLD} }  + \underbrace{ \mathbb{E}_{{q}_{\phi}(t|x,s)} \log{ {q}_{\phi}(m|t) }   }_{- Loss_{Classify} }  } \\
              \\
\end{array}</p>
...
{ - Loss_{KLD} } & {= - {D}_{KL}(q(t,c|x,s)||p(t,c)) } & {对应ELBO第二项} \\ 
...
最大化ELBO等价于最小化三种loss。其中，由于\\({D}_{KL}(q(t\|x,s)\|\|p(t))\\) 对于target cell，\\( t = 0 \\), \\(p(t) \sim Dirac \ distribution\\),无法计算KL divergence，使用Wasserstein distance代替KL divergence作为损失。
