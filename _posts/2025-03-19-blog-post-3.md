...
{}            &   { = \int q(c,t|x,s) \log{  p(x|c,t,s) dcdt}  +  \int  q(c,t|x,s)  \log{ \frac{ p(c,t|s) }{ q(c,t|x,s) } dcdt}  +  \int  q(t|x,s) \log{  q(m|t)   dt }   } \\
 ...
 {}            &   { = \underbrace{\mathbb{E}_{q(c,t|x,s)} \log{ p(x|c,t,s)} }_{ - Loss_{reconst} (- MSE(x,\hat{x})) } + \underbrace{\left( { - {D}_{KL}(q(c,t|x,s)||p(c,t)) } \right)}_{ - Loss_{KLDivergence} }  + \underbrace{ \mathbb{E}_{q(t|x,s)} \log{ q(m|t) }   }_{- Loss_{Classify}} ( - CE (m,\hat{m}) ) } \\
              \\
\end{array}</p>
