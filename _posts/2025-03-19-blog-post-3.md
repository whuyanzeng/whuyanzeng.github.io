...
{}              &  { = p(x|c, t, s) \ p(c|s) \ p(t|s) } \\
                    \\
{ q(c,t,m|x,s) }   &  { = q(c,t|x,s,m) \ q(m|x,s)   } \\
{}                &  { = q(t|x,s,m) \ q(c|x,s) \ q(m|x,s) } \\
                 \\
...
{}            &   { = \mathbb{E}_{q(c,t,m|x,s)} \log{ p(x|c,t,s)} - \int q(c,t,m|x,s) \log \frac{ q(t|x,s,m) \ q(c|x,s) \ q(m|x,s) }{ p(c|s) \ p(t|s) } dcdtdm } \\
{}           &   { = \mathbb{E}_{q(c,t,m|x,s)} \log{ p(x|c,t,s) } - {D}_{KL}(q(c|x,s)||p(c|s)) - {D}_{KL}(q(t|x,s,m)||p(t|s))  -  \int q(m|x,s) \log{ q(m|x,s) }  dm   } \\
              \\
