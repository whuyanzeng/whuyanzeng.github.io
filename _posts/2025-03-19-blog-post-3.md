...
{}             &   { \geq  \int q(c,t,m|x,s)  \log{p(x|c,t,s)} dcdtdm -  \int q(c,t|x,s) \log{ \frac{  q(c,t|x,s) }{ p(c,t|s) } } dcdt +  \int  q(t|x,s) \log{ q(m|t)} dt} & { \textb{ELBO} }\\
...
<p>\begin{array}{ll}
 { {D}_{KL}(q(t,c|x,s)||p(t,c)) } &{  = \left\{ 
                                              {\begin{array}{l} 
                                                ...
                                                {  {D}_{KL}(q(c|x,s)||p(c)) +  \textb{W}^{2}(q(t|x,s), \delta (t=0) )} \, \ {if\ cell\ n\ is non-targeting\ cell } \\
                                              }
                                        \right.
                            } \\
  {   \textb{W}^{2}(q(t|x,s), \delta (t=0) )   } & {  =  \sum{ \sigma_{t}(x,s)^{2} } + \sum{ \mu_{t}(x,s)^{2} }  } \\
\end{array}</p>
