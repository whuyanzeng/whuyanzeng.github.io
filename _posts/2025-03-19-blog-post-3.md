...
<p>\begin{array}{ll}
{ \log{ p(x|s)} + \log{q(m|x,s)} }   &  { = \log{\int p(x,c,t|s)dcdt}  + \log{\int q(m|t) \ q(t|x,s) \ q(t|x,s) dt} } \\
{}             &   { = \log{ \int q(c,t,m|x,s)  \frac{p(x|c,t,s) \ p(c,t|s) }{q(c,t,m|x,s)} dcdtdm } + \log{ \int   \ q(m|t) \ q(t|x,s) \ q(t|x,s) dt }  } \\
{}            &   { = \log{ \int q(c,t,m|x,s)  p(x|c,t,s) dcdtdm  - \log{ \int q(c,t,m|x,s)  \frac{ q(m|t) \ q(c|x,s) \ q(t|x,s) }{ p(c|s) \ p(t|s) } dcdtdm}  +  \log{ \int  \  q(m|t) \ q(t|x,s) \ q(t|x,s) dt }   } \\
{}            &   { = \log{ \int q(c,t,m|x,s)  p(x|c,t,s) dcdtdm - \log{ \int q(c,t|x,s)  \frac{ q(c|x,s) \ q(t|x,s) }{ p(c|s) \ p(t|s) } dcdt} - \log{ \int q(m|t) \ q(t|x,s) \  q(m|t)  dtdm} +    \log{ \int \ q(m|t) \ q(t|x,s) \ q(t|x,s) dt} } \\
{}            &   { = \log{ \int q(c,t,m|x,s)  p(x|c,t,s) dcdtdm - \log{ \int q(c,t|x,s)  \frac{ q(c|x,s) \ q(t|x,s) }{ p(c|s) \ p(t|s) } dcdt} - \log{ \int  q(t|x,s)   dt} +    \log{ \int  \ q(m|t) \ q(t|x,s) \ q(t|x,s) dt } } \\
{}            &   { = \log{ \int q(c,t,m|x,s)  p(x|c,t,s) dcdtdm - \log{ \int q(c,t|x,s)  \frac{ q(c|x,s) \ q(t|x,s) }{ p(c|s) \ p(t|s) } dcdt} + \log{ \int  q(m|t) dt}  } \\
\end{array}</p>

<p>\begin{array}{ll}
{}             &   { \geq  \int q(c,t,m|x,s)  \log{p(x|c,t,s)} dcdtdm -  \int q(c,t|x,s) \log{ \frac{ q(c|x,s) \ q(t|x,s) }{ p(c|s) \ p(t|s) }} dcdt +  \log{ \int q(m|t) dt } } & { ELBO }\\
{}            &   { = \mathbb{E}_{q(c,t,m|x,s)} \log{ p(x|c,t,s)} - \int q(c,t|x,s) \log{ \frac{  q(c|x,s) \ q(t|x,s) }{ p(c|s) \ p(t|s) } dcdt} +  \mathbb{E}_{q(c,t,m|x,s)} \log{ q(m|t) } } \\
{}           &   { = \mathbb{E}_{q(c,t,m|x,s)} \log{ p(x|c,t,s) } - {D}_{KL}(q(c|x,s)||p(c|s)) - {D}_{KL}(q(t|x,s)||p(t|s))  +  \mathbb{E}_{q(c,t,m|x,s)} \log{ q(m|t) }  } \\
{}           &   { = \mathbb{E}_{q(c,t,m|x,s)} \log{ p(x|c,t,s) } - {D}_{KL}(q(c|x,s)||p(c)) - {D}_{KL}(q(t|x,s)||p(t))  + \mathbb{E}_{q(c,t,m|x,s)} \log{ q(m|t) } } \\
{}           & { = - Loss_{reconst}- Diver_{KL} - Loss_{Classify}  } \\
...
<p>\begin{array}{ll}
{ Diver_{KL} }     & {= KL(q((t,c)|x)||p(t,c)) } \\ 
{ Loss_{reconst} } & {= MSE(x,\hat{x})} \\ 
{ Loss_{Classify}} & {= CE (m,\hat{m}) } \\ 
\end{array}</p>
