...
优化模型参数\\(\mathbf{W}\_{k \times g}\\)和\\(\mathbf{M}\_{c \times g}\\)使得最小化MSE，即 \\[\mathop{\arg\min}\limits_{W,M} \ \ \sum_{i=1}^{n} \sum_{j=1}^{g} u_{i,j}^{2}\\] 


## 正则化操作的意义
### \\(L_1\\)正则化(Lasso正则化)
在损失函数中，除了MSE项，额外加入了 \\(\sum_{i=1}^{k} \sum_{j=1}^{g} \| w_{i,j} \|\\) ， 使得得到的基因的权重矩阵 \\(\mathbf{W}\_{k \times g}\\) 较为稀疏，倾向于挑选出少量正交的具有差异的基因。
### \\(L_2\\)正则化(Ridge正则化)
在损失函数中，除了MSE项，额外加入了 \\(\sum_{i=1}^{k} \sum_{j=1}^{g} w_{i,j}^2\\) ，使得得到的对于基因的权重矩阵  \\(\mathbf{W}\_{k \times g}\\) 中包含大量非零的极小值，倾向于认为每个基因都受到微量的扰动。
### \\(L_1 + L_2\\)正则化(elastic-net正则化)
在损失函数中，除了MSE项，额外加入了 \\(\sum_{i=1}^{k} \sum_{j=1}^{g} \| w_{i,j} \| + \sum_{i=1}^{k} \sum_{j=1}^{g} w_{i,j}^2\\) ，使得得到的对于基因的权重矩阵 \\(\mathbf{W}\_{k \times g}\\) 中包含中等数量的非零值，结合了\\(L_1\\)正则化特征挑选的优势与\\(L_2\\)正则化兼顾全局的优势。
...
![gene clustered](https://oldcountlost.github.io/images/elastic_net_gene_cluster.png)

 \\(k\\) 个敲除的组，每个组作为一个pseudo-bulk，其表达量的umap为：
![KO group](https://oldcountlost.github.io/images/elastic_net_KO_cluster.png)
...
![gene program ~ KO group](https://oldcountlost.github.io/images/elastic_net_KO-program.de.png)
...
![gene clustered ~ KO group](https://oldcountlost.github.io/images/elastic_net_KO-gene_cluster.png)
...
![gene program ~ KO group p-value](https://oldcountlost.github.io/images/elastic_net_KO-program.png)
