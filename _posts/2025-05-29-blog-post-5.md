...
perturb-seq中\\(n\\)个细胞\\(g\\)个基因的的基因表达量数据为\\(\mathbf{E_{n \times g}}\\),基因敲除的组别数量为\\(k\\)，onehot编码的实验设计矩阵为\\(\mathbf{D_{n \times k}}\\)，那么对应的每个实验组对于基因表达的扰动矩阵为\\(\mathbf{W_{k \times g}}\\)，对于非实验因素，构成协变量矩阵\\(\mathbf{C_{n \times c}}\\)，并且匹配对应的扰动矩阵\\(\mathbf{M_{c \times g}}\\)。残差矩阵为\\(\mathbf{U_{n \times g}}\\)。
