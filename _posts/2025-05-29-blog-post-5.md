...
```
 ...
 \\(\mathbf{W})\\ 矩阵的每一个列作为一个向量，对g个向量进行无监督聚类，聚类为24个簇，其umap为：
![gene clustered ~ KO group](../../../../images/elastic_net_gene_cluster.png)

 \\(k)\\ 个敲除的组，每个组作为一个pseudo-bulk，其表达量的umap为：
![gene clustered ~ KO group](../../../../images/elastic_net_KO_cluster.png)

 \\(k)\\ 个敲除的组（x轴），基因的24个聚类簇（y轴），其平均表达量的热图为：
![gene clustered ~ KO group](../../../../images/elastic_net_KO-program.de.png)

 \\(k)\\ 个敲除的组（x轴），基因的24个聚类簇（y轴），其表达量的热图为：
![gene clustered ~ KO group](../../../../images/elastic_net_KO-gene_cluster.png)

 \\(k)\\ 个敲除的组（x轴），基因的24个聚类簇（y轴），其表达量的差异显著性图：
![gene clustered ~ KO group](../../../../images/elastic_net_KO-program.png)
